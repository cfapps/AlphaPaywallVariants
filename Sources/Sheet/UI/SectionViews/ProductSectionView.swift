...
final class ProductSectionView: UIView {
    ...
    var items: [ProductSectionView.CollectionViewCellModel] = [] {
        didSet {
            collection.update(sections: [
                QuickCollectionViewSection(items: items)
            ])
    ...
    private func setupUI() {
        ...
        if items.count > 0 {
            ...
            if let index = selectedItemIndex, index < items.count {
                collectionView.selectItem(
                    at: IndexPath(item: index, section: 0),
                    animated: false,
                    scrollPosition: []
...
extension ProductSectionView: UICollectionViewDelegate {
        ...
        override var isSelected: Bool {
            didSet {
                if isSelected {
                    ...
                    selectedContainerBackgroundView.layer.backgroundColor = UIColor.clear.cgColor
...
extension ProductSectionView.CollectionViewCell: QuickCollectionViewCellProtocol {
    
    func update(model: QuickCollectionViewCellModelProtocol) {
        guard let model = model as? ProductSectionView.CollectionViewCellModel else {
            return
        }
